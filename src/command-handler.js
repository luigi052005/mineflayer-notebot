...
//main functions
function load(prefix = "%", directory = "./src/commands/") {
...
function execute(bot, cmd, username, args, ...custom) {
  ...
  if (!is_initialized)
    return error(`The command handler was not initialized!`, "not_init");
...
function reload(command) {
  ...
  if (command == null) {
    ...
    try {
      ...
      console.log(`Error while reloading ${command}!`);
      console.log(err.stack);
...
module.exports.directory = "./src/commands";
