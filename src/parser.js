...
function tuneNoteblocks(file, noteblocks_1, instruments_enabled = false, callback = null) {
  ...
  const noteblocks = util.clone(noteblocks_1);
  const info = songInfo(file);
  if(info == null) {
  ...
  const pitches = info.used_pitches;
  const tuned = [];
  const untuned = [];
  const tuneLater = [];
  const nbByInst = info.noteblocksByInstrument;
