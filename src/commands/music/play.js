const name = "play";
const aliases = ["p"];
const description = "Play a notebot song";
const usage = "{prefix}play <song>";
const enabled = true;
...
function execute(bot, cmd, username, args, handler) {
	...
	const test = file.match(/\\/g);
	test == null ? 0 : test;

	if(file.match(/\//g).length > 2 || test > 0) {// so people don't try to go outside the ./songs/ directory
	...
	parser.tuneNoteblocks(file, noteblocks, true, (success, error) => {
		if(success) {
			...
			util.wait(200).then(() => {
				const songName = args[0].replace(".txt", "");
